...
def merge_datasets():
    ...
    # Save merged dataset (overwriting the original)
    output_path = Path("data/02_final_unified/unified_fwi_dataset_2010_2017.nc")
    print(f"\nSaving merged dataset to: {output_path}")
    print("(This will overwrite the existing file)")
    
    # Create backup of original first
    backup_path = output_path.with_suffix('.nc.backup')
